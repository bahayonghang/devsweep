...
/// Guidance returned when a legacy v1 plan is rejected.
pub const V1_RESCAN_MESSAGE: &str =
    "plan format v1 is no longer accepted; re-run `devsweep scan --json`";

#[derive(Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
/// Canonical identity of one trusted cleanup action footprint.
...
/// One target after rule reconstruction and trust-boundary validation.
...
impl ValidatedTarget {
    /// Returns the trusted in-memory target.
    pub(crate) fn target(&self) -> &CleanTarget {
        &self.target
    }

    /// Returns the built-in rule identifier.
    pub(crate) fn rule_id(&self) -> &str {
        &self.rule_id
    }

    /// Returns the canonical action fingerprint.
...
impl ValidatedPlan {
    /// Returns validated targets in plan order.
    pub(crate) fn targets(&self) -> &[ValidatedTarget] {
        &self.targets
    }

    /// Returns default-selected target identifiers in plan order.
...
/// Validates an untrusted persisted plan into an opaque execution input.
...
/// Revalidates an in-memory scanner plan through the persisted-plan trust boundary.
