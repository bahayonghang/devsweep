...
pub(crate) use cancel::NoopCancelObserver;
pub use cancel::{CancelObserver, FlagCancelObserver};
    ...
    pub(crate) fn process_fixture_exe() -> PathBuf {
        ENSURE_FIXTURE.call_once(|| {
            ...
            let target_dir = path
                ...
                .expect("process fixture path must be under <target>/<profile>");
            ...
            let status = Command::new(cargo)
                ...
                .status()
                .expect("spawn cargo build -p devsweep-cli --bin process_fixture");
            assert!(
                status.success(),
                "cargo build -p devsweep-cli --bin process_fixture failed with {status}"
            );
        });
        let path = fixture_path();
        assert!(
...
mod tests {
    ...
    fn hung_child_times_out_within_policy() {
        ...
        let request = base_request(&["hang"], &cancel, Duration::from_millis(300));
