...
/// Current read-only inventory report format version.
...
pub struct InventoryReport {
    /// Inventory report format version.
    pub version: u32,
    /// Root whose immediate contents were inventoried.
    pub root: PathBuf,
    /// Read-only capacity observations.
    pub observations: Vec<CapacityObservation>,
    /// Completeness and diagnostic observations.
    pub health: ScanHealth,
    #[serde(skip_serializing_if = "Option::is_none")]
    /// Optional inspect-only pnpm store finding.
    pub orphan_pnpm_store: Option<OrphanPnpmStoreFinding>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
/// Read-only capacity observation for one path.
pub struct CapacityObservation {
    /// Observed path.
    pub path: PathBuf,
    /// Whether the observation is informational or inspect-only.
    pub classification: InventoryClassification,
    /// Estimated byte count.
    pub estimated_bytes: u64,
    /// Whether the byte estimate is complete.
    pub size_complete: bool,
    /// Reasons why the estimate is incomplete.
    pub warnings: Vec<SizingWarning>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
/// Safety classification for an inventory observation.
pub enum InventoryClassification {
    /// Informational capacity observation with no cleanup semantics.
    InventoryOnly,
    /// Finding that requires manual inspection and cannot be selected.
    InspectOnly,
}

/// Inventories a root without creating cleanup targets.
