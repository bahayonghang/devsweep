...
pub struct OrphanPnpmStoreFinding {
    /// Safety classification for the finding.
    pub classification: InventoryClassification,
    /// Candidate project-local store path.
    pub candidate_path: PathBuf,
    /// Store path reported by pnpm configuration.
    pub configured_store: PathBuf,
    /// Bounded project-reference observations.
    pub project_references: Vec<PnpmProjectReference>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
/// Evidence describing one pnpm project reference file.
pub struct PnpmProjectReference {
    /// Reference file path.
    pub path: PathBuf,
    /// Whether the file references the candidate store.
    pub references_candidate: bool,
    /// Whether the file references the configured store.
