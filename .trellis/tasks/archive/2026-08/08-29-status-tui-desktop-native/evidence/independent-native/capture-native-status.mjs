// Independent native Status recapture. Isolated LOCALAPPDATA, new PIDs, loopback CDP.
...
const PRIVACY_RE = /cmdline|command[_\s-]?line|\buser\b|executable_path|process path/i;

log({
  event: "exe_hash",
  exe,
  sha256: sha256(exe),
  leftover_pids: execSync(
    "powershell.exe -NoLogo -NoProfile -Command \"@(Get-Process -Name devsweep-desktop -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Id) -join ','\"",
    { encoding: "utf8" },
  ).trim(),
});
...
async function waitForSnapshot(session, tag) {
  ...
  for (let i = 0; i < 50; i += 1) {
    ...
    state = await session.evaluate(`(() => {
      ...
      const rows = Array.from(document.querySelectorAll('.status-table tbody tr')).map((row) =>
        Array.from(row.querySelectorAll('td')).map((cell) => cell.textContent.trim()).slice(0, 4)
      );
      return {
        ...
        privacyHit: ${PRIVACY_RE}.test(text),
        chartGap: /\\bgap\\b|缺口/.test(text),
        firstProcess: rows[0] ?? null,
        headers: Array.from(document.querySelectorAll('.status-table thead button')).map((b) => b.textContent.trim()),
      };
    })()`);
...
{
  const session = await launch(null, "localappdata", 19671);
  ...
  writeFileSync(join(evidence, "snapshots", "status-en.txt"), await evaluate("document.body.innerText.replace(/\\s+/g,' ').slice(0, 2400)"), "utf8");
  ...
  const beforeSort = await evaluate(`(() => ({
    first: Array.from(document.querySelectorAll('.status-table tbody tr:first-child td')).map((c) => c.textContent.trim()),
    pressed: Array.from(document.querySelectorAll('.status-table thead button')).map((b) => ({ text: b.textContent.trim(), pressed: b.getAttribute('aria-pressed') })),
  }))()`);
  await evaluate(`(() => { Array.from(document.querySelectorAll('.status-table thead button')).find((b) => /Name|名称/.test(b.textContent || ''))?.focus(); })()`);
  const sortFocused = await evaluate(`(() => ({ tag: document.activeElement?.tagName, text: document.activeElement?.textContent, aria: document.activeElement?.getAttribute('aria-pressed') }))()`);
  log({ event: "keyboard_focus_sort_name", sortFocused, beforeSort });
  ...
  log({ event: "keyboard_sort_name", afterSort });
  ...
  for (let i = 0; i < 40; i += 1) {
    ...
    liveState = await evaluate(`(() => ({
      ...
      chartText: Array.from(document.querySelectorAll('.status-chart-values')).map((n) => n.textContent).slice(0, 2),
      gpuZero: /gpu 0/i.test(document.body.innerText || ''),
      cmdline: /cmdline/i.test(document.body.innerText || ''),
    }))()`);
  ...
  try {
    ...
    log({
      event: "axtree_captured",
      nodes: ax.nodes?.length ?? 0,
      privacyHit: PRIVACY_RE.test(axText),
      hasStopLive: /Stop live|停止实时/.test(axText),
      hasStartOrRefresh: /Start live|Refresh snapshot|开始实时|刷新快照/.test(axText),
      hasCmdline: /cmdline/i.test(axText),
    });
  } catch (error) {
  ...
  writeFileSync(join(evidence, "snapshots", "status-zh.txt"), await evaluate("document.body.innerText.replace(/\\s+/g,' ').slice(0, 2400)"), "utf8");
  for (const width of [1024, 800, 390]) {
    await send("Emulation.setDeviceMetricsOverride", { width, height: 860, deviceScaleFactor: 0, mobile: false });
    await sleep(700);
...
for (const scalePercent of [100, 125, 150, 200]) {
  const session = await launch(scalePercent, `localappdata-dpi-${scalePercent}`, 19680 + scalePercent);
  ...
  log({ event: "dpi_probe", scalePercent, pid: session.app.pid, ...dpr });
  await session.close();
}
