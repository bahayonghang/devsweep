// Independent Status resource gate on the release desktop binary.
// Idle snapshot-only, snapshot p95, 60 s explicit live, 25-sample post-exit
// evaluated against parent Status R4 / five-mode post-stop protocol.
...
function cpuBetween(a, b) {
  ...
  return 100 * (b.cpu_ms - a.cpu_ms) / dt;
}

function cpuSeries(samples) {
  const pcts = [];
  for (let i = 1; i < samples.length; i += 1) {
    const value = cpuBetween(samples[i - 1], samples[i]);
    if (value !== null) pcts.push(value);
  }
  return pcts;
}
...
async function launch(cdpPort) {
  ...
  return { app, send, evaluate, pid: app.pid, leftover };
}
...
const cores = Number(execSync('powershell.exe -NoLogo -NoProfile -Command "(Get-CimInstance Win32_ComputerSystem).NumberOfLogicalProcessors"', { encoding: "utf8" }).trim());
const ram = Number(execSync('powershell.exe -NoLogo -NoProfile -Command "(Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory"', { encoding: "utf8" }).trim());

const session = await launch(19771);
...
const idleSamples = samplePid(session.pid, 10000, 200, join(resourceDir, "idle.json"));
writeFileSync(join(resourceDir, "idle.json"), JSON.stringify(idleSamples, null, 2));
const idlePresent = idleSamples.filter((s) => s.present);
const idlePrivate = idlePresent.map((s) => s.private);
const idleThreads = idlePresent.map((s) => s.threads);
const idleCpu = cpuSeries(idlePresent);
const idlePrivateMedian = pct(idlePrivate, 0.5) ?? 0;
const idlePrivateMax = idlePrivate.length ? Math.max(...idlePrivate) : 0;
const idleThreadsMedian = pct(idleThreads, 0.5) ?? 0;
const idleThreadsMax = idleThreads.length ? Math.max(...idleThreads) : 0;
const idleCpuMedian = pct(idleCpu, 0.5) ?? 0;
...
const postSamples = samplePid(session.pid, 5000, 200, join(resourceDir, "post-exit.json"));
...
const idlePlus64 = idlePrivateMax + 64 * MiB;
const idlePlus4 = idleThreadsMax + 4;
const idleCpuCap = idleCpuMedian + 0.5;
const idleThreadCap = idleThreadsMax + 1;
const missingSample = postSamples.length !== 25;
const finalFive = postSamples.slice(-5);
const finalFiveEval = finalFive.map((sample, index) => {
  const previous = index === 0
    ? postSamples[postSamples.length - 6] ?? { present: false, wall_ms: sample.wall_ms - 200, cpu_ms: 0 }
    : finalFive[index - 1];
  const cpu = cpuBetween(previous, sample);
  const threads = sample.present ? sample.threads : 0;
  return {
    present: sample.present,
    cpu,
    threads,
    cpu_ok: cpu !== null && cpu <= idleCpuCap,
    threads_ok: threads <= idleThreadCap,
  };
});
const finalFiveHold = !missingSample
  && finalFiveEval.length === 5
  && finalFiveEval.every((row) => row.cpu_ok && row.threads_ok);
...
const summary = {
  host: {
    ...
    total_physical_memory: ram,
    sha256: sha256(exe),
    exe,
    pid: session.pid,
    leftover_before_launch: session.leftover,
  },
  warmup,
