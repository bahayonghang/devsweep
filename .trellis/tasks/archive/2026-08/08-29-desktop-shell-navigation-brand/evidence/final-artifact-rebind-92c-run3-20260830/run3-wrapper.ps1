...
if ($PSVersionTable.PSVersion.Major -lt 7) { throw "run3 wrapper itself must execute under PowerShell 7" }
if ((Get-Process -Id $PID).Path -ne $pwsh) { throw "run3 wrapper executable path mismatch" }
if (Test-Path -LiteralPath $outerExitPath) { throw "run3 outer-exit already exists; refusing overwrite" }
if (-not (Test-Path -LiteralPath $helper -PathType Leaf)) { throw "run3 helper missing" }

Write-Output ("RUN3_WRAPPER_START={0}" -f (Get-Date).ToString("o"))
...
Write-Output ("RUN3_WRAPPER_FINISHED={0}" -f $completedAt)
exit $helperExit
