...
Inspected sources are stored under `research/prior-art-sources/`. Do not execute
candidate scripts.
...
- `keep`: audit ≠ execute; evidence-backed recommendation table; dry-run/preview
  before any side effect; Chinese and English triggers; Trash vs reclaimed
...
- `invent`: bind the agent to DevSweep Clean/Analyze/Optimize/Status inspect
  commands; recommend-only default that never emits `execute` / `run` /
  `uninstall`; Production package under `skills/devsweep-inspect/`.
...
- Hypothesis: trigger evals will keep execute/uninstall requests from activating
  this skill. This remains missing evidence until `evals/trigger_cases.json` runs.
- Validated advantage: none yet.
