...
if (action === "launch") {
  ...
  const app = spawn(exe, [], {
    env: { ...process.env, LOCALAPPDATA: profile, WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS: `--remote-debugging-port=${port} --remote-allow-origins=* --disable-renderer-backgrounding --disable-backgrounding-occluded-windows --disable-background-timer-throttling${extra}` },
    stdio: "ignore",
    cwd: process.cwd(),
    detached: true,
    windowsHide: false,
