...
# Native analyze scan CPU probe for five-mode AC4 walk-window p95.
...
$RawDir = Join-Path $OutDir ('cpu-raw-' + [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssZ'))
...
Write-Host ("warmup elapsed={0}ms exit={1} p95={2} completeness={3}" -f $warmup.elapsed_ms, $warmup.exit, $warmup.cpu_p95, $warmup.completeness)

$runs = @()
for ($i = 1; $i -le 5; $i++) {
    Write-Host "measure #$i"
    $run = Invoke-AnalyzeSampled -Label "run$i" -Root $root
    Write-Host ("run{0} elapsed={1}ms exit={2} samples={3} p95={4} median={5} completeness={6}" -f $i, $run.elapsed_ms, $run.exit, $run.cpu_n, $run.cpu_p95, $run.cpu_median, $run.completeness)
    $runs += $run
}
...
$summaryPath = Join-Path $OutDir 'native-cpu-summary.json'
$json = $summary | ConvertTo-Json -Depth 10
[System.IO.File]::WriteAllText($summaryPath, $json, [System.Text.UTF8Encoding]::new($false))
Write-Host ("median_of_p95s={0} max_run_p95={1} all_le_200={2}" -f $summary.median_of_p95s, $summary.max_run_p95, $summary.all_run_p95_le_200)
