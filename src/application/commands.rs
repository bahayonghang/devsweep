...
use super::cli::{CleanCommand, InventoryCommand, ProtectCommand, ScanCommand};
use crate::{
...
pub(super) fn run_scan(command: ScanCommand) -> Result<()> {
...
pub(super) fn run_inventory(command: InventoryCommand) -> Result<()> {
...
pub(super) fn run_clean(command: CleanCommand) -> Result<()> {
...
pub(super) fn run_protect(command: ProtectCommand) -> Result<()> {
    match command {
...
pub(super) fn run_rules() -> Result<()> {
    let catalogue = crate::rules::rule_catalogue();
    println!("Project rules");
    for doc in catalogue
        .iter()
        .filter(|doc| doc.scope == RuleScope::Project)
    {
        println!("  {}", crate::rules::rule_row(doc));
    }
    println!();
...
mod tests {
    ...
    fn scan_report_saved_plan_decodes_to_its_embedded_untrusted_plan() {
        ...
        let report = ScanReport::new(UntrustedPlan::empty(), crate::model::ScanHealth::complete());
        fs::write(
            file.path(),
